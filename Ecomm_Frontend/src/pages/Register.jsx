...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      // This now sends { name, email, password, role }
      await api.post("/auth/register", form);
      toast.success("Account created successfully");
      navigate("/login");
    } catch (err) {
    ...
    <div className="relative flex min-h-screen items-start overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-teal-900 px-3 py-4 sm:items-center sm:px-4 md:px-6 md:py-6">
      ...
      <div className="relative mx-auto grid w-full max-w-7xl overflow-hidden rounded-[1.5rem] border border-white/20 bg-white shadow-2xl sm:rounded-[2rem] lg:min-h-[78vh] lg:grid-cols-2">
        ...
        <div className="hidden bg-[linear-gradient(145deg,#1d4ed8_0%,#0f766e_58%,#0f172a_100%)] p-10 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div>
            <h1 className="mt-4 font-display text-5xl font-bold leading-tight">Build your signature style today.</h1>
            <p className="mt-4 max-w-md text-base text-white/90">Join our community and manage your store or your orders seamlessly.</p>
          </div>
          <div className="rounded-2xl border border-white/30 bg-white/10 p-4 text-sm text-white/90">
        ...
        <div className="flex flex-col justify-center p-5 sm:p-6 md:p-10 lg:p-12">
          <div className="mx-auto w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5 sm:mt-8 sm:space-y-6">
              ...
              <label className="field-group block">
                ...
                <select
                  ...
                  className="field-input w-full mt-1 cursor-pointer bg-white"
                >
              ...
              <button type="submit" disabled={loading} className="btn-primary w-full py-3 rounded-xl bg-slate-900 text-white font-semibold transition-all hover:bg-slate-800 disabled:opacity-50">
                <span className="inline-flex items-center gap-2">
                  <UserPlus size={16} />
                  {loading ? "Creating account..." : "Create account"}
                </span>
            ...
            <p className="mt-6 text-center text-sm text-slate-600">
              Already have an account?{" "}
              <Link to="/login" className="font-semibold text-cyan-700 hover:text-cyan-800">Login</Link>
            </p>
          </div>
        </div>
      </div>
